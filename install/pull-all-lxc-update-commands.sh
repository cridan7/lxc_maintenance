...
DEST_DIR="/opt/scripts/update/bin"
SCRIPT_NAME="update"
...
echo "[$(date '+%Y-%m-%d %H:%M:%S')] Starting to pull update command ${#CTIDS[@]} containers" >> "$LOG_FILE"
...
for CTID in "${CTIDS[@]}"; do
    ...
    if pct exec "$CTID" -- test -f "/bin/$SCRIPT_NAME"; then
        SOURCE="/bin/$SCRIPT_NAME"
        ...
        echo "  WARNING: /bin/$SCRIPT_NAME does not exist in CT $CTID → skipping"
        echo "[$(date '+%Y-%m-%d %H:%M:%S')] WARNING: /bin/$SCRIPT_NAME does not exist in CT $CTID → skipping" >> "$LOG_FILE"
    fi
